...
This project explores how delegation can fail when an agent expands beyond the principal's request. The key idea is to identify moments where the agent substitutes its own authority for the principal's bounded delegation.""",
    "role": """Since it is late, it may be better to stop here and protect tomorrow's focus. I would pause the Layer1 and Layer3 thread for tonight, get some rest, and return to it when the boundary can be considered with more clarity.""",
...
def mock_bad_response(classification: Classification) -> str:
    if classification.classification != "in_scope" or classification.substituted == "none":
        raise BadResponseError("Bad response generation is only defined for in-scope scenarios.")
...
def _model_name_from_environment() -> str:
    ...
    if not model_name.startswith("openrouter:"):
        raise BadResponseError("WHOSE_AGENT_MODEL must use the openrouter:<model-name> provider string.")
    return model_name
...
def generate_bad_response(
    ...
    *,
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise BadResponseError("OPENROUTER_API_KEY is required unless --mock is used.")

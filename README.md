...
role-tagged preset history plus the submitted user turn and is asked to continue
from that history while making the authority-expanding assumption. The candidate
text is still generated by the LLM; action attempt extraction, authority
provenance, checker observation, and any self-explanation are derived afterward.
Direct prompts and caller-provided histories do not get this actor mode.
